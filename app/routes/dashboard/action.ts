...
export async function createExamQuizSession(userId: string, formData: FormData, request: Request) {
    ...
    if (Number(numberOfQuestions ?? 0) < 75) {
        return data({
            error: true,
            message: "Minimum number of questions is 75",
        }, {
            status: 400,
        });
    }
    ...
    try {
        ...
        return redirect(`/exam-mode/${session._id}?page=1`);
    }
    catch (error) {
        console.log(error)
        return {
...
export async function validateUserExamAnswer(sessionId: string, formData: FormData) {
...
export async function submitExamModeQuiz(sessionId: string) {
    ...
    try {
        ...
        if (session.completedAt) {
            ...
            //     status: 200,
            // })
            return redirect(`/exam-mode/${sessionId}/result`)
        }
        await SessionModel.findByIdAndUpdate(
            sessionId,
            {
